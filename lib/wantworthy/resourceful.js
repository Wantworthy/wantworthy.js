          ...
          if(_.isArray(attrs._embedded[resourceName])) {
            self[resourceName]  =  _.map(attrs._embedded[resourceName], function(r) {
              if(resourceful.resources[resourceName]) {
                ...
                return new resourceful.Resource(r);
              }
            });
          } else {
            var resourceData = attrs._embedded[resourceName];
            if(resourceful.resources[resourceName]) {
              self[resourceName] = new(resourceful.resources[resourceName])(resourceData);
            } else {
              return new resourceful.Resource(resourceData);
            }
          }
        } else {
